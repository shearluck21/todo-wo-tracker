// Feature-complete TodoList with stable due dates, daily rollover, pinned-while-editing,
// priority sorting (High → Medium → Low), and cleanup of past-done tasks.
// Designed to be compile-safe on Vercel.
...
type Task = {
  ...
  // Stable due fields (store these; UI label is derived at render time)
  dueKey?: string | null;     // "today" | "tomorrow" | "next-mon" | "wd-YYYY-MM-DD"
  dueISO?: string | null;     // "YYYY-MM-DD" local date

  // Staged (pending) due change while editing; we commit on "row complete" (notes blur)
...
/** Build selectable due options from "now". Labels ALWAYS carry an explicit date so the pill text stays fresh. */
function buildDueOptions(now = new Date()) {
  ...
  // Fri/Sat/Sun → Next Monday only (skip weekend)
  ...
  // Mon..Thu: Tomorrow + subsequent weekdays up to Fri + Next Monday
...
export default function TodoList() {
  ...
  const [tasks, setTasks] = useState<Task[]>(() => {
    const loaded = typeof window !== "undefined" ? loadTasks() : [];
  ...
  // On mount: one-time cleanup of done tasks from past days
  useEffect(() => {
    ...
    if (last !== todayIso) {
      setTasks((prev) =>
        prev.filter((t) => {
          ...
          if (!t.dueISO) return true; // undated tasks are not auto-deleted
          return t.dueISO >= todayIso; // keep today & future; prune past
        })
      );
  ...
  // Schedule a tick just after local midnight: prune old dones + re-render so group labels refresh automatically
  useEffect(() => {
    const now = new Date();
    const next = new Date(now);
    next.setHours(24, 0, 0, 50); // 00:00:00.050
    ...
    const id = setTimeout(() => {
      ...
      setLastCleanupISO(todayIso);
      // No need to recompute labels; they’re derived from dueISO at render time.
    }, ms);
    return () => clearTimeout(id);
  }, []);

  // First focus to the first rendered row
  useEffect(() => {
  ...
  // Global hotkey "n" to add a task (ignored inside inputs)
  useEffect(() => {
  ...
  const addInlineTask = () => {
    ...
    pinnedNewIdRef.current = t.id; // keep it at the very top while editing
  ...
  const endRowEditing = (id: string | null) => {
    ...
    if (!id && pinnedNewIdRef.current) {
      // If the pinned row has staged due changes, commit them now
  ...
  const setPriorityFor = (id: string, p: Priority) => {
    ...
    // Keep new/pinned at top until row completes
    pinnedNewIdRef.current = id;
  };

  const stageDueFor = (id: string, key: string) => {
    // Translate selected key → iso/label
    const opt =
      buildDueOptions().find((o) => o.key === key) ||
      buildDueOptions()[0];
    setTasks((prev) =>
      prev.map((t) =>
        t.id !== id
          ...
          : {
              ...t,
              // Stage values; grouping will update only when row completes
  ...
  const orderLabels = useMemo(() => {
    ...
    for (const k of Object.keys(groups)) {
      if (!set.has(k)) dynamic.push(k);
    }
    return [...fixed, ...dynamic].filter((k) => groups[k]?.length);
  }, [groups]);

  // Build stable index map by current array order (used to make priority sort stable)
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-6 sm:px-8">
      ...
      {orderLabels.map((label) => {
        ...
        // Keep a pinned-new row at the top of its group while editing
...
function TodoRow({
  ...
  const currentKey = t.pendingDueKey ?? t.dueKey ?? computeKeyForISO(t.dueISO);
      ...
      <div className="space-y-1">
        ...
        <div className="pl-7 flex items-center gap-2 text-[11px]">
          ...
          <select
            ...
            className="rounded-md border px-2 py-1 text-[11px] font-mono border-blue-200 bg-blue-50/70 text-blue-800"
            aria-label="Due date"
          >
            {dueOpts.map((o) => (
...
// Feature-complete TodoList with stable due dates, daily rollover, priority and due "pills", inline select, and safe TS.
"use client";
...
function InlineSelect<T extends string>({
  ...
  return (
    <span className={`inline-flex rounded-md bg-white/60 border border-neutral-200 ${className}`}>
      {options.map((opt) => (
        <button
          key={opt.value}
          type="button"
          ...
          className={
            `px-2 py-1 text-xs font-mono transition rounded-md
            ${opt.value === value
              ? "font-semibold ring-1 ring-inset ring-neutral-300"
              : "opacity-70 hover:opacity-100"}
            ${opt.className ?? ""} ${pillClass}`
          }
          style={opt.value === value ? { background: "rgba(0,0,0,0.04)" } : {}}
...
function TodoRow({
  ...
  const dueOpts = buildDueOptions();
  const currentKey = t.pendingDueKey ?? t.dueKey ?? computeKeyForISO(t.dueISO);
  const priorityOpts: { value: Priority; label: string; className: string }[] = [
    { value: "high", label: "high", className: "border-red-200 bg-red-50/70 text-red-800" },
    { value: "medium", label: "med", className: "border-amber-200 bg-amber-50/70 text-amber-800" },
    { value: "low", label: "low", className: "border-neutral-200 bg-white/70 text-neutral-800" },
  ];
  return (
    <TodoCard data-row={t.id} tabIndex={0} className="focus:outline-none">
      <div className="space-y-1">
        ...
        <div className="pl-7 flex items-center gap-2 text-[11px]">
          ...
          <InlineSelect
            ...
            ariaLabel="Due date"
            pillClass="max-w-[120px] truncate"
          />
          <div className="ml-auto flex items-center gap-2">
            <GlassButton onClick={() => onSetEditingId(isEditing ? null : t.id)} size="sm" data-action-toggle={t.id}>
              {isEditing ? "Done" : "Edit"}
